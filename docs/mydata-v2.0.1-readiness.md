...
- `11.1` means **ΑΛΠ** and `11.2` means **ΑΠΥ**. Each listing has its own default document type (`11.2` or `2.1`), which a reservation may explicitly override. OTA settlement examples use `2.1` (service invoice) to the merchant of record, with `vatCategory=2` (13%) and `E3_561_007` / `category1_3`.
